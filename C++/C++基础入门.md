...
int main() {
	...
	int * p = arr;  //指向数组的指针  数组名即数组的首地址

	cout << "第一个元素： " << arr[0] << endl;
	cout << "指针访问第一个元素： " << *p << endl;   //*p解引用后即是数组的第一个元素
...
int main() {
	...
	int len = sizeof(arr) / sizeof(int);  //或者/sizeof(arr[0])
...
结构体属于<u>用户自定义的数据类型</u>，允许用户存储不同的数据类型
...
struct student
{
	...
	int age;      //年龄
	int score;    //分数
}stu3; //结构体变量创建方式3            stu1\2\3是具体的变量
...
int main() {
	...
	printStudent(&stu);  //传入地址   一个指针只占4个字节
